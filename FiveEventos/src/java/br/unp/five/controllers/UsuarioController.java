...
public class UsuarioController implements Serializable {
    ...
    @EJB
    private br.unp.five.facade.UsuarioFacade ejbFacade;
    ...
    public String prepareCreateNew() {
        current = new Usuario();
        selectedItemIndex = -1;
        return "/faces/Create";
    }

    public HttpSession getSessionUser() {

        sessionUser = (HttpSession) FacesContext
                .getCurrentInstance()
                .getExternalContext()
                .getSession(true);
    ...
    public String create() {
        ...
        try {
            if (current.getIdUsuario() == null) {
                current.setIdUsuario(0);
                String senha = Criptografia.MD5(current.getSenha());
    ...
    public String update() {
        try {
            ...
            String senha = Criptografia.MD5(current.getSenha());
            current.setSenha(senha);
            Integer idUser = (Integer) getSessionUser().getAttribute("idUser");
            if (idUser.equals(current.getIdUsuario())) {
                getFacade().edit(current);

                JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/resources/Bundle").getString("UsuarioUpdated"));
            } else {
                JsfUtil.addErrorMessage("Usuario sem permissao!");
            }
            return "View";
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/resources/Bundle").getString("PersistenceErrorOccured"));
